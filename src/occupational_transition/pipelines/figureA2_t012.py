...
def _figure_a2_layout(root: Path) -> FigureA2Layout:
    fig = root / "figures"
    inter = root / "intermediate"
    return FigureA2Layout(
        root=root,
        raw=root / "raw" / "sipp",
        fig=fig,
        inter=inter,
...
def run(root: Path) -> None:
    build_figure_a2_sipp_event_study(_figure_a2_layout(root))
...
def download_zip(release_year: int, raw: Path) -> tuple[Path, bytes, str]:
...
def load_peio_to_occ22(cross: Path) -> dict[str, int]:
...
def load_tercile_by_occ22(terciles: Path) -> dict[int, str]:
...
def ingest_panel_sqlite(
    ...
    """
    zip_path, zip_bytes, zip_url = download_zip(release_year, layout.raw)
    ...
    db_path = layout.inter / f"_sipp_panel_{release_year}_{uuid.uuid4().hex}.sqlite"
    csv_temp = layout.inter / f"_sipp_pu_{release_year}_{uuid.uuid4().hex}.csv"
    ...
    return {
        ...
        "zip_sha256": zip_hash,
        "zip_cached_path": str(zip_path.relative_to(layout.root)).replace("\\", "/"),
        "csv_member": csv_name,
...
def build_figure_a2_sipp_event_study(layout: FigureA2Layout) -> None:
    ...
    peio_map = load_peio_to_occ22(layout.cross)
    tercile_by_occ22 = load_tercile_by_occ22(layout.terciles)

    agg: dict[tuple[int, str], list[float]] = defaultdict(lambda: [0.0, 0.0, 0.0, 0.0])
    sources: list[dict[str, Any]] = []

    for yr in PANEL_RELEASE_YEARS:
        meta = ingest_panel_sqlite(yr, peio_map, tercile_by_occ22, agg, layout)
        sources.append(meta)
    ...
    layout.meta_json.write_text(json.dumps(meta_out, indent=2), encoding="utf-8")
