"""PR-000: build occ22 and sector6 crosswalks and refresh managed data_registry rows."""
...
def build_occ22_rows(raw: Path) -> list[dict[str, str]]:
...
def build_sector6_rows(raw: Path) -> list[dict[str, str]]:
    rows: list[dict[str, str]] = []
...
# Rows whose snapshot_download_date PR-000 refreshes (rest of registry preserved).
PR000_MANAGED_REGISTRY_IDS: frozenset[str] = frozenset(
    {
        "census_tp78_table_a1_a2",
        "census_cps_occupation_codes_pdf",
        "bls_soc_2018_structure",
        "bls_jt_industry",
        "bls_ce_supersector",
        "bls_ce_industry",
        "bls_bd_industry",
        "census_btos_api_strata",
        "bls_qcew_industry_titles",
    }
)


def write_registry(docs: Path, snapshot_date: str) -> None:
    """
    Refresh snapshot dates for PR-000-managed registry rows only.

    The full ``docs/data_registry.csv`` (including OEWS/O*NET rows) is the
    source of truth in git; PR-000 must not truncate it.
    """
    docs.mkdir(parents=True, exist_ok=True)
    reg_path = docs / "data_registry.csv"
    if not reg_path.is_file():
        raise FileNotFoundError(
            f"Missing {reg_path}; restore from git before running PR-000."
        )
    df = pd.read_csv(reg_path, dtype=str)
    if "dataset_id" not in df.columns:
        raise ValueError("data_registry.csv must contain dataset_id column")
    mask = df["dataset_id"].isin(PR000_MANAGED_REGISTRY_IDS)
    df.loc[mask, "snapshot_download_date"] = snapshot_date
    df.to_csv(reg_path, index=False)


def run(root: Path) -> None:
    """PR-000 build entrypoint for ``run_step``."""
    raw = root / "raw"
    ...
    sec_rows = build_sector6_rows(raw)
    sfields = [
        "source_program",
        "source_code",
        "source_label",
    ...
    write_registry(docs, snapshot_date)
