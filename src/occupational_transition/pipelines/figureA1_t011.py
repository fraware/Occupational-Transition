...
def _figure_a1_layout(root: Path) -> FigureA1Layout:
    fig = root / "figures"
    inter = root / "intermediate"
    raw = root / "raw" / "cps" / "asec"
    return FigureA1Layout(
        root=root,
        raw=raw,
        fig=fig,
        inter=inter,
...
def run(root: Path) -> None:
    build_figure_a1_asec_welfare(_figure_a1_layout(root))
...
def load_dtocc_to_occ22(cross: Path) -> dict[int, int]:
    cross_df = pd.read_csv(cross)
    sub = cross_df[cross_df["source_system"] == "CPS_PRDTOCC1"].copy()
    if sub.empty:
...
def build_figure_a1_asec_welfare(layout: FigureA1Layout) -> None:
    ...
    dtocc_to_occ22 = load_dtocc_to_occ22(layout.cross)
