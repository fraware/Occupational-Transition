"""T-004: Figure 2 Panel B weighted transition counts (adjacent CPS months)."""
...
from occupational_transition.pipelines import figure2_panelA_t003 as bf2a
...
def run(root: Path) -> None:
    """T-004 build entrypoint for ``run_step``."""
    ...
    for (y1, m1), (y2, m2) in pairs:
        dat1, meta1 = bf2a.ensure_month_dat_file(
            y1, m1, raw=raw, repo_root=root
        )
        dat2, meta2 = bf2a.ensure_month_dat_file(
            y2, m2, raw=raw, repo_root=root
        )
