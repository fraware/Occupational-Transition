...
def _figure_a9_layout(root: Path) -> FigureA9Layout:
    fig = root / "figures"
    inter = root / "intermediate"
    return FigureA9Layout(
        root=root,
...
def run(root: Path) -> None:
    build_figure_a9_acs_local_composition(_figure_a9_layout(root))
...
def build_figure_a9_acs_local_composition(layout: FigureA9Layout) -> None:
    ...
    if not layout.cross.is_file():
        raise RuntimeError(f"missing crosswalk: {layout.cross}")
    if not layout.terciles.is_file():
        raise RuntimeError(f"missing ai terciles: {layout.terciles}")

    crosswalk_df = pd.read_csv(layout.cross)
    ...
    local_zip = layout.raw / f"acs_pums_{acs_year}_{suffix}_{PUMS_PERSON_ZIP_NAME}"
    ...
    meta = {
        ...
        "crosswalk_sha256": sha256_file_stream(layout.cross),
        "ai_terciles_file": str(layout.terciles.relative_to(layout.root)).replace("\\", "/"),
        "ai_terciles_sha256": sha256_file_stream(layout.terciles),
        "source_files_sha256": [
            {
