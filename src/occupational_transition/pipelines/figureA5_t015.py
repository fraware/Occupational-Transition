...
def _figure_a5_layout(root: Path) -> FigureA5Layout:
    fig = root / "figures"
    inter = root / "intermediate"
    return FigureA5Layout(
        root=root,
        raw=root / "raw" / "bls" / "ce",
        fig=fig,
        inter=inter,
...
def run(root: Path) -> None:
    build_figure_a5_ces_payroll_hours(_figure_a5_layout(root))
...
def build_figure_a5_ces_payroll_hours(layout: FigureA5Layout) -> None:
    ...
    sector6_labels = load_sector6_labels_from_crosswalk(layout.cross)
    ...
    df.to_csv(layout.out_csv, index=False)
    ...
    layout.out_meta.write_text(json.dumps(meta_out, indent=2), encoding="utf-8")
    print(f"Wrote {layout.out_csv} ({len(df)} rows)")
