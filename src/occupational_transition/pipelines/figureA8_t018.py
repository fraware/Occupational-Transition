...
def _figure_a8_layout(root: Path) -> FigureA8Layout:
    fig = root / "figures"
    inter = root / "intermediate"
    return FigureA8Layout(
        root=root,
...
def run(root: Path) -> None:
    build_figure_a8_lehd(_figure_a8_layout(root))
...
def build_figure_a8_lehd(layout: FigureA8Layout) -> None:
    ...
    index_raw = fetch_with_cache(
        US_INDEX_URL, layout.raw / "latest_release_us_index.html"
    )
    manifest_raw = fetch_with_cache(MANIFEST_URL, layout.raw / "j2jr_us_manifest.txt")
    data_raw = fetch_with_cache(
        DATA_URL, layout.raw / "j2jr_us_d_f_gn_n_oslp_s.csv.gz"
    )
