"""T-003: build Figure 2 Panel A hours by AI tercile (CPS Basic + T-002 terciles)."""
...
def ensure_month_dat_file(
    ...
    *,
    ...
    meta: dict[str, Any] = {
        ...
        "local_dat_path": str(dat_path.relative_to(repo_root)).replace("\\", "/"),
    }
    ...
    if zip_ok:
        ...
        meta["local_zip_path"] = str(zip_path.relative_to(repo_root)).replace("\\", "/")
    ...
    meta["dat_sha256"] = sha256_file(dat_path)
    meta["local_gzip_path"] = str(gz_path.relative_to(repo_root)).replace("\\", "/")
    return dat_path, meta
...
def weighted_mean_hours(
    dat_path: Path,
    positions: dict[str, tuple[int, int]],
    cross: Path,
    terciles: Path,
) -> pd.DataFrame:
    pr_map, ter = load_pr_occ22_tercile(cross, terciles)
...
def run(root: Path) -> None:
    """T-003 build entrypoint for ``run_step``."""
    raw = root / "raw" / "cps" / "basic"
    fig = root / "figures"
    inter = root / "intermediate"
    cross = root / "crosswalks" / "occ22_crosswalk.csv"
    terciles_path = inter / "ai_relevance_terciles.csv"
    ...
    for y, m in months:
        dat_path, asset_meta = ensure_month_dat_file(
            y, m, raw=raw, repo_root=root
        )
        layout_pos, layout_year, layout_url = load_layout_for_year(y)

        sub = weighted_mean_hours(dat_path, layout_pos, cross, terciles_path)
        if sub.empty:
