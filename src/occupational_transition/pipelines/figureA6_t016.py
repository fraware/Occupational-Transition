...
def _figure_a6_layout(root: Path) -> FigureA6Layout:
    fig = root / "figures"
    inter = root / "intermediate"
    return FigureA6Layout(
        root=root,
        raw=root / "raw" / "bls" / "bd",
        fig=fig,
        inter=inter,
...
def run(root: Path) -> None:
    build_figure_a6_bed_churn(_figure_a6_layout(root))
...
def resolve_series(
    ...
    for sec in SECTOR6_ORDER:
        ...
        for measure_key, dataclass_code in MEASURE_MAP.items():
            hits: list[str] = []
            for r in series_rows:
                ...
                if r.get("sizeclass_code") != "00":
                    continue
                if r.get("dataclass_code") != dataclass_code:
                    continue
...
def build_figure_a6_bed_churn(layout: FigureA6Layout) -> None:
    ...
    sector_labels = load_sector6_labels_from_crosswalk(layout.cross)
    ...
    for fname in META_FILES:
        ...
        (layout.raw / fname).write_bytes(raw)
        if fname == "bd.series":
    ...
    (layout.raw / payload_name).write_bytes(payload_raw)
    ...
    pd.DataFrame(out_rows, columns=OUT_COLS).to_csv(layout.out_csv, index=False)
    ...
    print(f"Wrote {layout.out_csv} ({len(out_rows)} rows)")
