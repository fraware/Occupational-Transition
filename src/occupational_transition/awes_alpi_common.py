"""Shared helpers for AWES / ALPI-style metrics (pure functions, no I/O).

Normalization and crosswalk utilities align with ``build_crosswalks`` and the
figure build scripts. This module lives under ``src/`` so pipelines, QA, and
installed users share one implementation; ``scripts/awes_alpi_common.py`` is
a thin shim for legacy ``from awes_alpi_common import ...`` in repo scripts.
"""
...
# Frozen six-sector NAICS2 mapping (aligned with scripts/build_crosswalks.py).
...
def naics_string_to_n2(
    ...
    if naics_field is None:
        return None
    if isinstance(naics_field, float) and np.isnan(naics_field):
        return None
...
def naics2_to_sector6(n2: int) -> tuple[str | None, str, str]:
    ...
    if n2 == 62:
        return (
            ...
            "naics2_health_social_assistance",
        )
    if n2 in (44, 45):
