"""QA for Figure 4 Panel A JOLTS sector rates (T-008)."""
...
def main() -> int:
    ...
    if len(retained) != 24:
        errors.append(f"retained_series must list 24 series, got {len(retained)}")

    if meta.get("crosswalk_file") != "crosswalks/sector6_crosswalk.csv":
        errors.append(
            "metadata crosswalk_file must be crosswalks/sector6_crosswalk.csv"
        )

    file_entries = meta.get("source_files_sha256") or []
    if len(file_entries) < 8:
        errors.append("metadata source_files_sha256 must include all T-008 inputs")
    ...
    for entry in file_entries:
        ...
        if not name or not url or not expected:
            ...
            continue
        try:
            got = _fetch_sha256(url, root=root, local_name=name)
        except Exception as e:
