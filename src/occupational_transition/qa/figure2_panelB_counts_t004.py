"""Ticket QA (from scripts/qa_figure2_panelB_counts.py)."""
...
ALLOWED_STATES = {f"occ22_{i:02d}" for i in range(1, 23)} | {"unemployed", "nilf"}
...
def main() -> int:
    ...
    MATCH_REGIME_CSV = (
        root / "intermediate" / "figure2_panelB_match_regime_robustness.csv"
    )
    MISSING_MONTH_SENS_CSV = (
        root / "intermediate" / "figure2_panelB_missing_month_sensitivity.csv"
    )
    errors: list[str] = []
    ...
    if str(df["month"].min()) != "2019-01":
        errors.append(
            "series must start at 2019-01 for origin month, got " f"{df['month'].min()}"
        )
    ...
    if months_avail and str(months_avail[0]) != "2019-01":
        errors.append(
            "months_available must start at 2019-01, got " f"{months_avail[0]}"
        )
    ...
    if MATCH_REGIME_CSV.is_file():
        ...
        if set(mr.get("regime", pd.Series(dtype=str)).astype(str).unique()) != {
            ...
            errors.append(
                "match regime robustness must include strict/moderate/permissive"
            )
