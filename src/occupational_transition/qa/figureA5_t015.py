"""Ticket QA (from scripts/qa_figureA5_ces_payroll_hours.py)."""
...
def main() -> int:
    ...
    if sectors != EXP_SECTORS:
        errors.append(
            f"sector set mismatch: expected {sorted(EXP_SECTORS)}, got {sorted(sectors)}"
        )
    ...
    if not bad.empty:
        errors.append(
            f"each month must have 6 sectors; offending months: {bad['month'].tolist()[:12]}"
        )
    ...
    for entry in hashes:
        ...
        if got != exp:
            errors.append(
                f"sha256 mismatch for {name}: expected {exp[:16]} got {got[:16]}"
            )
