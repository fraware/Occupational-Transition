"""Ticket QA (from scripts/qa_figure2_panelB_probs.py)."""
...
def main() -> int:
    ...
    # Matrix row checks
    if not mat.empty:
        bad_dest = mat["destination_state"].isna() | (mat["destination_state"] == "")
        if bad_dest.any():
        ...
        if low < -ROW_SUM_TOLERANCE or high > 1.0 + ROW_SUM_TOLERANCE:
            errors.append(
                "matrix transition_probability out of [0,1]: " f"min={low} max={high}"
            )
    ...
    if not summ.empty:
        ...
        if bad_m:
            errors.append(f"invalid summary metric_name values: {sorted(bad_m)}")
