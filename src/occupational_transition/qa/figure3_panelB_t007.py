"""Ticket QA (from scripts/qa_figure3_panelB_btos_workforce_effects.py)."""
...
def main() -> int:
    ...
    if meta.get("universe_scope_code") != 2:
        errors.append(
            "metadata universe_scope_code must be 2 " "(AI-using firms, last 6 months)"
        )
    ...
    if "census.gov" not in src.lower() or "AI_Supplement_Table" not in src:
        errors.append(
            "primary_source_url must point to Census AI_Supplement_Table.xlsx"
        )
    ...
    if meta.get("fallback_reason") in (None, ""):
        errors.append("fallback_reason must document primary vs fallback outcome")
