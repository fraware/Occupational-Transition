"""Ticket QA (from scripts/qa_figureA3_cps_supp_validation.py)."""
...
def main() -> int:
    ...
    if len(df) != 3:
        errors.append(
            f"output must have exactly 3 rows (low/middle/high), got {len(df)}"
        )
    ...
    if got_order != TERCILE_ORDER:
        errors.append(
            f"ai_relevance_tercile rows must be ordered {TERCILE_ORDER}, got {got_order}"
        )
    ...
    if not isinstance(meta.get("sources"), list) or len(meta["sources"]) < 2:
        errors.append(
            "metadata sources must include jan24pub.csv and cpsjan24.pdf entries"
        )
    ...
    if not jan_csv.is_file():
        ...
        src_csv = next(
            (s for s in meta["sources"] if s.get("file_name") == "jan24pub.csv"), None
        )
    ...
    if not jan_pdf.is_file():
        ...
        src_pdf = next(
            (s for s in meta["sources"] if s.get("file_name") == "cpsjan24.pdf"), None
        )
    ...
    if "crosswalks/occ22_crosswalk.csv" not in lin:
        ...
        if (
            ...
            and _sha256_file(crosswalk_path) != lin["crosswalks/occ22_crosswalk.csv"]
        ):
    ...
    if "intermediate/ai_relevance_terciles.csv" not in lin:
        ...
        if (
            terciles_path.is_file()
            and _sha256_file(terciles_path)
            != lin["intermediate/ai_relevance_terciles.csv"]
        ):
