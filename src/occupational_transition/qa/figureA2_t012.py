"""Ticket QA (from scripts/qa_figureA2_sipp_event_study.py)."""
...
def main() -> int:
    ...
    by_k = df.groupby("event_time")["ai_relevance_tercile"].apply(
    ...
    if (df["mean_snap_participation"] < 0).any() or (
        df["mean_snap_participation"] > 1
    ).any():
    ...
    for k, grp in df.groupby("event_time", sort=False):
        ...
        if got != exp_tercile_order:
            errors.append(
                f"event_time {k}: rows must be ordered {exp_tercile_order}, got {got}"
            )
    ...
    if not META_JSON.is_file():
        ...
        if ry != [2022, 2023, 2024]:
            errors.append(
                f"metadata sipp_panel_release_years must be [2022,2023,2024], got {ry}"
            )
