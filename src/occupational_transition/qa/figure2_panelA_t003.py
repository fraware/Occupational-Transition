"""Ticket QA (from scripts/qa_figure2_panelA.py)."""
...
def main() -> int:
    ...
    by_m = df.groupby("month")["ai_relevance_tercile"].apply(
    ...
    if low < 1 or high > 99:
        errors.append(
            f"mean_usual_weekly_hours out of 1..99 range: min={low} max={high}"
        )
    ...
    if META_JSON.is_file():
        ...
        months_in_csv = (
            df.drop_duplicates(subset=["month"], keep="first")["month"]
            .astype(str)
            .tolist()
        )
