...
Large microdata files (CPS, ACS PUMS, NLSY, etc.) are **not** committed; they are downloaded by scripts under `raw/` per [docs/replication.md](docs/replication.md). Those datasets have **separate terms of use** from the Census Bureau, BLS, and NLS.
...
For Git object database size and removing accidental large blobs from history, see [docs/git_history_hygiene.md](docs/git_history_hygiene.md).
