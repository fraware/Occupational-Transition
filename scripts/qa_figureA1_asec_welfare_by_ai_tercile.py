"""QA for Figure A1 ASEC welfare by AI tercile (T-011). Exit 1 on failure."""
...
META_JSON = ROOT / "intermediate" / "figureA1_asec_welfare_by_ai_tercile_run_metadata.json"
...
def main() -> int:
    ...
    by_y = df.groupby("year")["ai_relevance_tercile"].apply(lambda s: set(s.astype(str)))
    ...
    for y, grp in df.groupby("year", sort=False):
        ...
        if got != exp_tercile_order:
            errors.append(f"year {y}: rows must be ordered {exp_tercile_order}, got {got}")

    if (df["poverty_rate"] < 0).any() or (df["poverty_rate"] > 1).any():
        errors.append("poverty_rate must be in [0, 1]")
    if (df["unemployment_incidence"] < 0).any() or (df["unemployment_incidence"] > 1).any():
    ...
    if not META_JSON.is_file():
        ...
        if years_sorted != ry:
            errors.append(f"CSV years {years_sorted} must match metadata retained_years {ry}")
