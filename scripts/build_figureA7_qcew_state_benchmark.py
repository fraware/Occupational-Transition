...
def load_qcew_sector6_map() -> dict[str, tuple[str, str]]:
...
def main() -> None:
    ...
    sector_map = load_qcew_sector6_map()

    page_path = RAW / "downloadable-data-files.html"
    ...
    qtrly_raw = fetch_with_cache(qtrly_url, RAW / qzip_name)
    ...
    cross_sha = sha256_bytes(CROSS.read_bytes())
    meta = {
        "ticket": "T-017",
        ...
        "source_files_sha256": [
            {
                ...
                "local_cache_path": str(page_path.relative_to(ROOT)).replace("\\", "/"),
            },
            {
                "file_name": qzip_name,
                "url": qtrly_url,
                "sha256": sha256_bytes(qtrly_raw),
                "local_cache_path": str((RAW / qzip_name).relative_to(ROOT)).replace(
                    "\\", "/"
                ),
            },
        ],
