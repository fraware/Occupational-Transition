"""
QA for Figure A4 ABS structural adoption output and metadata.
"""
...
META_JSON = ROOT / "intermediate" / "figureA4_abs_structural_adoption_run_metadata.json"
...
def main() -> None:
    ...
    if df["source_table_id"].nunique() != 1 or str(df["source_table_id"].iloc[0]) != "ABSTCB2018":
    ...
    if not any("census.gov/programs-surveys/abs/data/tables.html" in u for u in source_urls):
    ...
    for a in artifacts:
        ...
        if p.endswith("raw/abs/abstcb_2018_us_national.json"):
            ...
            break
    if json_art is None:
        errors.append("metadata downloaded_artifacts missing local ABS API json artifact")
    else:
