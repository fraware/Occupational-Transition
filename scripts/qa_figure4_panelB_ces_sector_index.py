"""QA for Figure 4 Panel B CES sector payroll index (T-009). Exit 1 on fail."""
...
def main() -> int:
    ...
    try:
        ...
        df["index_aug2023_100"] = pd.to_numeric(
            df["index_aug2023_100"], errors="raise"
        )
    ...
    if len(base) != 6:
        errors.append(
            f"expected 6 rows for base month {BASE_MONTH}, got {len(base)}"
        )
    else:
        bad_idx = base["index_aug2023_100"] - 100.0
        if (
            bad_idx.abs() > INDEX_TOL
        ).any():
            errors.append(
                f"index_aug2023_100 must be 100 in {BASE_MONTH} for each "
                "sector"
            )
    ...
    if len(retained) != 6:
        errors.append(
            f"retained_series must list 6 series, got {len(retained)}"
        )
    ...
    if len(file_entries) < 8:
        errors.append(
            "metadata source_files_sha256 must include all T-009 inputs"
        )
