...
**Repository state for this revision:** manuscript numbers and examples below are aligned with commit `72f02bf2e7897f5515ab9212f9e6fe6fbcd2c432` (short `72f02bf`). Replace with a [release tag](../replication/project_maintenance.md#results-freeze-and-tagging) such as `results-YYYY-MM-DD` when you publish a labeled freeze.
...
**Main descriptive result.** We find a **stable weekly-hours gap** between AI-relevance terciles: in **2026-01** weighted mean usual weekly hours are about **40.4** (high), **37.4** (middle), and **36.3** (low)—roughly **four hours** between high and low (`figures/figure2_panelA_hours_by_ai_tercile.csv`). For transition summaries, origin-mass-weighted dashboard KPIs (same evidence window) report **unemployment-entry** and **NILF-entry** shares on the order of **0.7%** and **1.6%** for high-origin mass in **2026-01** (`figures/memo_dashboard_kpis.csv`; construction `docs/quality/README.md#memo-visuals-t-101-to-t-108-precision-and-non-invention-rules`). These are **descriptive** matched-month constructs, not causal AI effects.

**Data and construction.** T-003 averages usual weekly hours by month and tercile. T-004 builds transition counts in a coarse state space (22 occupation groups plus unemployment and NILF). T-005 row-normalizes to probabilities and produces summary metrics. Panel B combines a **latest-month** transition-count heatmap with time series of retention, occupation switching, unemployment entry, and NILF entry. See `docs/methodology/tickets/t003_*` through `docs/methodology/tickets/t005_*` for windows and weight handling.
...
**Policy recommendations** (CPS/BTOS modules, scoped JOLTS supplements, NLS items) are **measurement priorities**, not causal payoffs estimated here. Keep abstract and conclusion language consistent with that split per [claim_audit.md](../policy/claim_audit.md).
...
**Design.** The analysis is **descriptive**: official weights where applicable, frozen crosswalks, and ticketed build/QA scripts. Full construction, universes, time windows, and limitations appear in [methods_data.md](methods_data.md) and per-ticket `docs/methodology/tickets/t*_methodology.md`. **Authoritative URLs and snapshot fields** are recorded in [data_registry.csv](../data_registry.csv).

**Replication.** Environment, download behavior, and commands are in [replication README](../replication/README.md). Full clean rebuild: `python scripts/run_full_pipeline_from_raw.py` (see [README.md](../../README.md)). Acceptance criteria: [acceptance_matrix.md](../replication/acceptance_matrix.md). **Evidence freeze:** examples and percentages in this manuscript were checked against commit **`72f02bf`** (`72f02bf2e7897f5515ab9212f9e6fe6fbcd2c432`); substitute an annotated tag (e.g. `results-YYYY-MM-DD`) when you publish a labeled release.
...
The appendix deepens the main text in a fixed order: worker annual welfare (A1), SIPP medium-run adjustment (A2), CPS supplement validation (A3), ABS structural adoption (A4), CES payroll and hours (A5), BED churn (A6), QCEW state benchmark (A7), LEHD public benchmark (A8), **ACS local occupational composition (A9)**—supporting geographic scope for Claim 1 when needed—and NLSY97 long-run careers (A10). Draft paragraphs and figure paths are in [appendix_draft.md](appendix_draft.md); methodological detail is in `docs/methodology/tickets/t011_*` through `docs/methodology/tickets/t020_*`. Visual stems are `t011`–`t020` per [figure_catalog.md](../figures/figure_catalog.md).

---

## Policy and briefing artifacts (repository)

Optional senator and Virginia-facing materials (`docs/policy/briefing/senate_briefing_*.md`, memo stems `t101`–`t108`, Virginia `va01`–`va08`) are **additive** to this manuscript; they are descriptive monitoring aids, not causal estimates. See [paper_draft_v1_public_data_ai_labor.md](paper_draft_v1_public_data_ai_labor.md) for entry points.
...
- U.S. Census Bureau / BLS. Current Population Survey (CPS); American Community Survey (ACS); Business Trends and Outlook Survey (BTOS); Annual Business Survey (ABS); SIPP. See registry and `docs/figures/source_notes/fig0*_sources.md`.
- U.S. Department of Labor / O*NET. O*NET database and Work Activities (Importance scale); see registry rows for O*NET downloads.
- National Longitudinal Surveys (NLS). NLSY97 public-use files; see `docs/methodology/tickets/t020_figureA10_nls_longrun_methodology.md` and registry.

Per-figure source notes: [../figures/source_notes/fig01_sources.md](../figures/source_notes/fig01_sources.md) through [fig05_sources.md](../figures/source_notes/fig05_sources.md).
...
- [x] [claim_audit.md](../policy/claim_audit.md) caveats reflected: Claim 1 geography → appendix A9 (§2, §9); Claim 3 **partial** support and **proxy-explicit** BTOS Panel B language (abstract, §4, §9); Claim 5 **empirical diagnosis** vs **policy judgment** (abstract, §6, §9).
...
- [x] §8 **Replication** cites [replication README](../replication/README.md), [acceptance_matrix.md](../replication/acceptance_matrix.md), and **concrete** commit `72f02bf` / full hash.
