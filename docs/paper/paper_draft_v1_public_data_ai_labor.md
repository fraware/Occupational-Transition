...
**Figure discussion.** Panel A reports national employment shares and median annual wages by the 22-group occupation taxonomy. Panel B reports standardized O*NET task intensities and the AI Task Index used for deterministic tercile assignment. This section is descriptive baseline measurement, not causal inference. See `docs/figures/captions/fig01_caption.md` and `docs/figures/memos/fig01.md`.
...
**Figure discussion.** Panel A reports weighted mean usual weekly hours by month and AI-relevance tercile. Panel B combines a latest-month transition-count heatmap with time-series probabilities for retention, occupation switching, unemployment entry, and NILF entry. This section is descriptive matched-month survey monitoring, not causal attribution to AI adoption. See `docs/figures/captions/fig02_caption.md` and `docs/figures/memos/fig02.md`.
...
**Dataset sentence.** Full reproducibility is defined by `docs/replication/README.md` and `python scripts/run_full_pipeline_from_raw.py`.

**Figure discussion.** Main-text figures align with Claims 1–5 subject to `docs/policy/claim_audit.md` (Claim 1 in main text is scoped to employment, wages, and task content; geography is appendix evidence).
...
Consolidated methods: `docs/paper/methods_data.md`. Registry: `docs/data_registry.csv`.

## Policy and briefing artifacts (repository)

The same pipeline outputs support policy-facing materials that are **additive** to this manuscript: senator memo narrative, frozen Virginia evidence, lineage tables, hearing Q&A, one-page handout, print packet order, and optional memo visuals (`t101`–`t108`) plus Virginia visuals (`va01`–`va08`). Primary entry points: `docs/policy/briefing/senate_briefing_memo.md`, `docs/policy/briefing/senate_briefing_evidence_baseline_va.md`, `docs/policy/briefing/senate_briefing_lineage_va.md`, `docs/policy/briefing/senate_briefing_script_va.md`, `docs/policy/briefing/senate_briefing_qa_va.md`, `docs/policy/briefing/senator_handout_1page_va.md`, `docs/policy/briefing/senator_packet_order_va.md`. Technical summary and rebuild commands: `docs/policy/briefing/virginia_deep_dive.md`; figure and stem catalog: `docs/figures/figure_catalog.md`; orchestrated build/QA: `scripts/run_memo_visuals_build.py`, `scripts/run_memo_visuals_qa.py` (see `docs/replication/README.md`). Senator-facing claims are logged in `docs/policy/claim_audit.md` (Senator brief claim ledger). These artifacts are descriptive monitoring support, not causal estimates.

## Appendix
Order and draft paragraphs: `docs/paper/appendix_outline.md`, `docs/paper/appendix_draft.md`.

## Reproducibility and integrity
- Full clean rebuild: `scripts/run_full_clean_rebuild_acceptance.py` (invoked by `run_full_pipeline_from_raw.py`).
- Acceptance matrix: `docs/replication/acceptance_matrix.md`.
- Claim audit: `docs/policy/claim_audit.md`.
- Robustness reports: `scripts/run_robustness_all.py` outputs under `intermediate/robustness/`.
- Results freeze procedure: `docs/replication/project_maintenance.md#results-freeze-and-tagging`; manifest: `scripts/build_freeze_manifest.py`.
