# Virginia deep dive (case study module)
...
The last three lines require **optional** scripts that may not exist or may be gitignored in your clone; the **tracked** minimum is the T-017 build plus `build_state_qcew_deep_dive.py` / `qa_state_qcew_deep_dive.py`.
...
- Static charts from `scripts/visualize_virginia_memo.py` (memo visual pack; map to briefing labels **va01**–**va08**):
  - `docs/states/virginia/visuals/png/virginia_sector_composition.png` (+ `docs/states/virginia/visuals/vector/virginia_sector_composition.pdf`) — va01
  - `docs/states/virginia/visuals/png/virginia_sector_wages.png` (+ matching PDF) — va02
  - `docs/states/virginia/visuals/png/virginia_peers_sector_shares.png` (+ matching PDF) — va03
  - `docs/states/virginia/visuals/png/virginia_peers_sector_wages.png` (+ matching PDF) — va04
  - `docs/states/virginia/visuals/png/virginia_sector_ranks.png` (+ matching PDF) — va05
  - `docs/states/virginia/visuals/png/virginia_kpi_dashboard.png` (+ matching PDF) — va06
  - Optional when source inputs exist: `virginia_btos_state_highlight` (va07), `virginia_occ_context` (va08)

If you still have older builds under `visuals/png/` and `visuals/vector/`, move those `virginia_*` (or `va*`) files into `docs/states/virginia/visuals/png/` and `docs/states/virginia/visuals/vector/` respectively.
...
## Senator-facing documentation (often local-only)

Narrative memo, Q&A, handouts, and packet order for legislative use may live under `docs/policy/briefing/` in workflows that keep those files **out of git** (see repository `.gitignore`). When present, typical paths include:
...
| `docs/policy/claim_audit.md` | Senator brief claim ledger (Virginia package); **tracked** |

**Full memo + Virginia build in one pass** (when local orchestration scripts exist): `python scripts/run_memo_visuals_build.py` then `python scripts/run_memo_visuals_qa.py` (see [replication/README.md](../../replication/README.md)).

**Case study index:** [README.md](README.md).
