...
The repo ships **separate** senator- and staff-facing documents and visuals built from the same reproducible `figures/` and `intermediate/*_run_metadata.json` lineage as the empirical stack. They do not replace the paper; they translate measurement limits and Virginia-relevant benchmarks into briefing-ready form. Entry points (often local-only under `docs/policy/briefing/`): `senate_briefing_memo.md` (full narrative, executive summary, visual decision table, federal action matrix, non-claims), `senate_briefing_evidence_baseline_va.md` (frozen headline values), `senate_briefing_lineage_va.md` (metric-to-source appendix), `senate_briefing_script_va.md`, `senate_briefing_qa_va.md`, `senator_handout_1page_va.md`, `senator_packet_order_va.md`. Virginia QCEW-derived tables and `va01`–`va08` stems (tracked): `docs/states/virginia/virginia_deep_dive.md`, `docs/figures/figure_catalog.md`. State case study hub: `docs/states/README.md`. Build/QA: `scripts/run_memo_visuals_build.py` and `scripts/run_memo_visuals_qa.py` (`docs/replication/README.md`). Claim discipline for testimony: `docs/policy/claim_audit.md` (Senator brief claim ledger). Treat these outputs as **descriptive monitoring** only, not causal AI effect estimates.
